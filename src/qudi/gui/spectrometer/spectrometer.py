...
class SpectrometerGui(GuiBase):
    ...
    def on_activate(self):
        ...
        if not self._image_exposure_time:
            self._image_exposure_time = self.spectrometer_logic().exposure_time
        if not self._image_readout_speed:
            self._image_readout_speed = self.spectrometer_logic().readout_speed
        if not self._alignment_exposure_time:
            self._alignment_exposure_time = self.spectrometer_logic().exposure_time
        if not self._spectrum_exposure_time:
            self._spectrum_exposure_time = self.spectrometer_logic().exposure_time
        if not self._spectrum_readout_speed:
            self._spectrum_readout_speed = self.spectrometer_logic().readout_speed
    ...
    def _activate_settings_tab(self):
        ...
        spectro_constraints = self.spectrometer_logic().spectro_constraints
        ...
        for i in range(3):
            if i < len(self.spectrometer_logic().spectro_constraints.gratings):
                self._grating_buttons[i].setText('{}rpm'.format(
                    round(self.spectrometer_logic().spectro_constraints.gratings[i].ruling/1000)))
                self._grating_buttons[i].setCheckable(True)
                self._grating_buttons[i].clicked.connect(partial(self._manage_grating_buttons, i))
                if i == self.spectrometer_logic().grating:
        ...
        for i in range(2):
            ...
            if i<len(self._input_ports):
                ...
                if self._input_ports[i].type.name == self.spectrometer_logic().input_port:
                ...
                input_widget.setValue(self.spectrometer_logic().get_input_slit_width(self._input_ports[i].type.name))
            ...
            if i < len(self._output_ports):
                ...
                if self._output_ports[i].type.name == self.spectrometer_logic().output_port:
                ...
                output_widget.setValue(self.spectrometer_logic().get_output_slit_width(self._output_ports[i].type.name))
        ...
        self._calibration_widget.setValue(self.spectrometer_logic().wavelength_calibration)
        self._calibration_widget.setSuffix('m')
        self._settings_tab.calibration_layout.addWidget(self._calibration_widget)

        for gain in self.spectrometer_logic().camera_constraints.internal_gains:
            self._settings_tab.camera_gains.addItem(str(gain), gain)
            if gain == self.spectrometer_logic().camera_gain:
                self._settings_tab.camera_gains.setCurrentText(str(gain))

        for trigger_mode in self.spectrometer_logic().camera_constraints.trigger_modes:
            self._settings_tab.trigger_modes.addItem(trigger_mode, trigger_mode)
            if trigger_mode == self.spectrometer_logic().trigger_mode:
        ...
        self._center_wavelength_widget.setValue(self.spectrometer_logic().center_wavelength)
        ...
        self._mw.center_wavelength_current.setText("{:.2r}m".format(ScaledFloat(self.spectrometer_logic().center_wavelength)))
        ...
        if not self.spectrometer_logic().camera_constraints.has_shutter:
            self._settings_tab.shutter_modes.setEnabled(False)
        else:
            self._settings_tab.shutter_modes.setCurrentText(self.spectrometer_logic().shutter_state)
            self._settings_tab.shutter_modes.currentTextChanged.connect(self.set_settings_params)

        if self.spectrometer_logic().camera_constraints.has_cooler:

            self._settings_tab.cooler_on.clicked.connect(self._manage_cooler_button)
            if self.spectrometer_logic().cooler_status:
            ...
            self._temperature_widget.setValue(self.spectrometer_logic().temperature_setpoint - 273.15)
            ...
            self._mw.camera_temperature.setText(
                "{}°C".format(round(self.spectrometer_logic().camera_temperature - 273.15, 2)))
        ...
        self.spectrometer_logic().sigUpdateSettings.connect(self._update_settings)

    def _activate_image_tab(self):
        """ Initialization method for the image tab """

        camera_width = self.spectrometer_logic().camera_constraints.width
        camera_height = self.spectrometer_logic().camera_constraints.height

        for read_mode in self.spectrometer_logic().camera_constraints.read_modes:
        ...
        for readout_speed in self.spectrometer_logic().camera_constraints.readout_speeds:
    ...
    def _activate_spectrum_tab(self):
        ...
        self._spectrum_scan_delay_widget.setValue(self.spectrometer_logic().scan_delay)
        ...
        self._spectrum_scan_wavelength_step_widget.setValue(self.spectrometer_logic().scan_wavelength_step)
        ...
        self._spectrum_tab.scan_number_spin.setValue(self.spectrometer_logic().number_of_scan)
    ...
    def _update_settings(self):
        ...
        self._manage_grating_buttons(self.spectrometer_logic()._grating)

        if len(self._input_ports)>1:
            input_port_index = 0 if self.spectrometer_logic()._input_port == PortType.INPUT_FRONT else 1
            self._manage_port_buttons(input_port_index)
            self._input_slit_width[input_port_index].setValue(self.spectrometer_logic()._input_slit_width[input_port_index])
        else:
            self._input_slit_width[0].setValue(self.spectrometer_logic()._input_slit_width[0])
        if len(self._output_ports)>1:
            output_port_index = 0 if self.spectrometer_logic()._output_port == PortType.OUTPUT_FRONT else 1
            self._manage_port_buttons(output_port_index+2)
            self._output_slit_width[output_port_index].setValue(self.spectrometer_logic()._output_slit_width[output_port_index])
        else:
            self._output_slit_width[0].setValue(self.spectrometer_logic()._output_slit_width[0])

        self._calibration_widget.setValue(self.spectrometer_logic().wavelength_calibration)
        self._settings_tab.camera_gains.setCurrentText(str(int(self.spectrometer_logic()._camera_gain)))
        self._settings_tab.trigger_modes.setCurrentText(self.spectrometer_logic()._trigger_mode)
        if self.spectrometer_logic().camera_constraints.has_cooler:
            self._temperature_widget.setValue(self.spectrometer_logic()._temperature_setpoint-273.15)

            cooler_on = self.spectrometer_logic()._cooler_status
        ...
        #if self.spectrometer_logic().camera_constraints.has_shutter:
        #    self._settings_tab.shutter_modes.setCurrentText(self.spectrometer_logic()._shutter_state)

        self._mw.center_wavelength_current.setText("{:.2r}m".format(ScaledFloat(self.spectrometer_logic().center_wavelength)))

    def set_settings_params(self):

        if self.spectrometer_logic().module_state() == 'locked':
            return

        self.spectrometer_logic().wavelength_calibration = self._calibration_widget.value()
        self.spectrometer_logic().camera_gain = self._settings_tab.camera_gains.currentData()
        self.spectrometer_logic().trigger_mode = self._settings_tab.trigger_modes.currentData()
        if self.spectrometer_logic().camera_constraints.has_cooler:
            self.spectrometer_logic().temperature_setpoint = self._temperature_widget.value()+273.15
        self.spectrometer_logic().shutter_state = self._settings_tab.shutter_modes.currentText()
    ...
    def set_image_params(self):
        ...
        self.spectrometer_logic().acquisition_mode = self._image_tab.acquisition_modes.currentData()
        self.spectrometer_logic().read_mode = self._image_tab.read_modes.currentData()
        self.spectrometer_logic().exposure_time = self.image_exposure_time_widget.value()
        self.spectrometer_logic().readout_speed = self._image_tab.readout_speed.currentData()

        self.spectrometer_logic()._update_acquisition_params()
        self._image_params = dict(self.spectrometer_logic().acquisition_params)
    ...
    def set_alignment_params(self):
        ...
        self.spectrometer_logic().acquisition_mode = "LIVE_SCAN"
        self.spectrometer_logic().read_mode = self._alignment_tab.read_modes.currentData()
        self.spectrometer_logic().exposure_time = self.alignment_exposure_time_widget.value()
        self.spectrometer_logic().readout_speed = max(self.spectrometer_logic().camera_constraints.readout_speeds)
    ...
    def set_spectrum_params(self):
        ...
        self._manage_tracks()
        self.spectrometer_logic().acquisition_mode = self._spectrum_tab.acquisition_modes.currentData()
        self.spectrometer_logic().read_mode = self._spectrum_tab.read_modes.currentData()
        self.spectrometer_logic().exposure_time = self.spectrum_exposure_time_widget.value()
        self.spectrometer_logic().readout_speed = self._spectrum_tab.readout_speed.currentData()
        self.spectrometer_logic().scan_delay = self._spectrum_scan_delay_widget.value()
        self.spectrometer_logic().number_of_scan = self._spectrum_tab.scan_number_spin.value()
        self.spectrometer_logic().scan_wavelength_step = self._spectrum_scan_wavelength_step_widget.value()

        self.spectrometer_logic()._update_acquisition_params()
        self._spectrum_params = dict(self.spectrometer_logic().acquisition_params)
    ...
    def start_acquisition(self, tab_index):
        ...
        self.spectrometer_logic().sigUpdateData.connect(partial(self._update_data, tab_index))
        ...
        self.spectrometer_logic().start_acquisition()

    def stop_acquisition(self):

        self.spectrometer_logic().stop_acquisition()
        self.spectrometer_logic().sigUpdateData.disconnect()
        self._manage_stop_acquisition()

    def save_image_data(self):

        filepath = self.savelogic().get_path_for_module(module_name='spectrometer')
        self.spectrometer_logic().savelogic().save_data({"wavelength":self._image_data['wavelength'].flatten(),
                                   "data":self._image_data['data'].flatten()}, filepath=filepath, parameters=self._image_params)

    def save_spectrum_data(self):

        filepath = self.savelogic().get_path_for_module(module_name='spectrometer')
        self.spectrometer_logic().savelogic().save_data({"wavelength":self._spectrum_data['wavelength'].flatten(),
    ...
    def _manage_grating_buttons(self, tab_index):
        ...
        for i in range(3):
            ...
            if i == tab_index:
                btn.setChecked(True)
                btn.setDown(True)
                self.spectrometer_logic().grating = i
            else:
    ...
    def _manage_port_buttons(self, tab_index):
        for i in range(2):
            if tab_index < 2:
                ...
                if i == tab_index:
                    self.spectrometer_logic().input_port = self._input_ports[i].type
                    btn.setChecked(True)
                    btn.setDown(True)
                else:
                    btn.setChecked(False)
                ...
                if i+2 == tab_index:
                    self.spectrometer_logic().output_port = self._output_ports[i].type
                    btn.setChecked(True)
                    btn.setDown(True)
                else:
                    btn.setChecked(False)
    ...
    def _manage_slit_width(self, tab_index):
        ...
        if tab_index < 2:
            self.spectrometer_logic().set_input_slit_width(self._input_slit_width[tab_index].value(), self._input_ports[tab_index].type)
        elif tab_index > 1:
            self.spectrometer_logic().set_output_slit_width(self._output_slit_width[tab_index-2].value(), self._output_ports[tab_index-2].type)

    def _manage_cooler_button(self):
        cooler_on = not self.spectrometer_logic().cooler_status
        self.spectrometer_logic().cooler_status = cooler_on
        self._settings_tab.cooler_on.setChecked(cooler_on)
        self._settings_tab.cooler_on.setDown(cooler_on)
        self._settings_tab.cooler_on.setText("ON" if not cooler_on else "OFF")
        self._mw.cooler_on_label.setText("Cooler {}".format("ON" if cooler_on else "OFF"))
    ...
    def _manage_center_wavelength(self):
        ...
        self.spectrometer_logic().center_wavelength = self._center_wavelength_widget.value()
        self._mw.center_wavelength_current.setText("{:.2r}m".format(ScaledFloat(self.spectrometer_logic().center_wavelength)))
    ...
    def _manage_tracks(self):
        ...
        if np.any(self.spectrometer_logic().active_tracks != active_tracks):
            self.spectrometer_logic().active_tracks = active_tracks

    def _manage_image_advanced(self):

        hbin = self._image_tab.horizontal_binning.value()
        vbin = self._image_tab.vertical_binning.value()
        self.spectrometer_logic().image_advanced_binning = [hbin, vbin]
        ...
        self.spectrometer_logic().image_advanced_area = image_advanced
    ...
    def _update_temperature(self):
        ...
        self._mw.camera_temperature.setText(str(round(self.spectrometer_logic().camera_temperature-273.15, 2))+"°C")
    ...
    def _update_data(self, tab_index):
        ...
        data = self.spectrometer_logic().acquired_data
        wavelength = self.spectrometer_logic().acquired_wavelength
        ...
        if tab_index == 0:
            ...
            if self.spectrometer_logic().read_mode == "IMAGE_ADVANCED":
                rectf = self._image_advanced_widget.parentBounds()
                self._image.setRect(rectf)
            else:
                width = self.spectrometer_logic().camera_constraints.width
                height = self.spectrometer_logic().camera_constraints.height
            ...
            counts = data.mean()
            x = self._counter_data[0]+self.spectrometer_logic().exposure_time
            ...
            if self.spectrometer_logic().acquisition_mode == 'MULTI_SCAN':
            ...
            if self.spectrometer_logic().read_mode == "MULTIPLE_TRACKS":
        ...
        if not self.spectrometer_logic().module_state() == 'locked':
            self.spectrometer_logic().sigUpdateData.disconnect()
            self._manage_stop_acquisition()